...
**Implementation status:** CONTRACT DEFINED; NUMERIC CEILINGS NOT YET
CALIBRATED\
...
``` text
GLOBAL_MAX = absolute safety ceiling

LOW_MAX    = conservative ceiling
BALANCED_MAX = medium ceiling
HIGH_MAX   = larger ceiling
```
...
Konsep sebelumnya yang disetujui adalah:
...
Dalam arsitektur tiered, angka 100 paling aman diposisikan sebagai:
...
``` text
LOW_MAX <= BALANCED_MAX <= HIGH_MAX <= 100
```

Angka tier-specific belum dikunci.
...
DO NOT add a minimum tile count.
...
**Document status:** REMINDER / ARCHITECTURE LOCK\
