...
window.CHANGELOG_DATA = [
 { version: `v90.2.170`, items: [
  { id: `Migrasi User_ID Tahap 4 tuntas untuk Produksi_GC & kartu KPI Member: statistik produksi member (Total Tonase/Waste/Hasil Bersih) sekarang dicari via User_ID dulu baru fallback Nama -- memperbaiki bug lama di mana data produksi member yang pernah ganti nama tercatat terpecah jadi 2 (tidak tergabung). Pilar 3 & 4 Engine KPI (Kelengkapan Sampling, Laporan Tepat Waktu) ikut disambungkan ke User_ID dengan pola sama. Semua perubahan backward-compatible (0 dampak kalau kolom User_ID belum terisi).`, en: `User_ID Migration Stage 4 completed for Produksi_GC & the Member KPI card: member production stats (Total Tonnage/Waste/Net Result) are now looked up via User_ID first with Name as fallback -- fixing a longstanding bug where a renamed member's production data was recorded as two separate, unmerged totals. KPI Engine Pillars 3 & 4 (Sampling Completeness, On-Time Reporting) were wired to User_ID using the same pattern. All changes are backward-compatible (zero impact where the User_ID column isn't yet populated).` },
  { id: `Backend Code.gs (7028 baris, 1 file) dipecah jadi 8 file terpisah (01_Config s/d 08_KPIEngine) untuk kerapian editor Apps Script -- murni pindah fungsi tanpa restrukturisasi logika apapun (doPost/doGet masih 1 fungsi utuh masing-masing, direncanakan dipecah lebih dalam di sesi terpisah). Diverifikasi diff byte-per-byte 100% identik dengan file sumber sebelum dipecah.`, en: `The backend Code.gs (7028 lines, 1 file) split into 8 separate files (01_Config through 08_KPIEngine) purely for Apps Script editor tidiness -- a pure function relocation with zero logic restructuring (doPost/doGet each remain a single monolithic function, planned for deeper splitting in a separate session). Verified byte-for-byte identical via diff against the pre-split source.` },
  { id: `Kartu Member (grid utama) sekarang punya tombol "Lihat selengkapnya" -- bagian Engine KPI 5 Pilar (5 badge pilar + Skor Gabungan + Konteks Periode) collapse secara default, card memanjang ke bawah saat dibuka (mendorong kartu lain di grid, tidak menutupi apa pun). Dipilih setelah 3 gaya tampilan dicoba langsung di aplikasi (panel geser dari kanan & card membesar di tempat tidak dipakai).`, en: `The Member card (main grid) now has a "View details" toggle -- the 5-Pillar KPI Engine section (5 pillar badges + Combined Score + Period Context) collapses by default, with the card expanding downward when opened (pushing other grid cards down, covering nothing). Chosen after live-testing 3 display styles in the app (a right-sliding panel and an in-place enlarging card were not used).` },
  { id: `Splash screen loading awal (opsional, default nonaktif) diganti total dari versi progress-bar+logo statis sebelumnya menjadi animasi "Pixel Boot": wordmark LITHOSITE jatuh huruf per huruf (2 huruf "I" jadi ikon pensil/bor, huruf "O" jadi logo gem Lithosite dengan efek glow oranye), disusul loader garis shimmer + 4 titik memantul. Latar navy gelap (sama dengan warna kartu di seluruh aplikasi), bukan hitam pekat. Durasi tetap bisa diatur 1-7 detik lewat slider di Developer Console > Technical (timing animasi menyesuaikan proporsional).`, en: `The optional initial-load splash screen (off by default) completely replaced the previous progress-bar-and-static-logo version with a "Pixel Boot" animation: the LITHOSITE wordmark drops in letter by letter (the two "I" letters become a pencil/drill icon, the "O" becomes the Lithosite gem logo with an orange glow effect), followed by a shimmer-bar-and-4-bouncing-dots loader. Background is dark navy (matching the card color used throughout the app), not solid black. Duration remains adjustable from 1-7 seconds via the slider in Developer Console > Technical (animation timing scales proportionally).` }
 ] },
 { version: `v90.2.160`, items: [
  { id: `Rebranding penuh dari "Geobank"/"Geobank Minerals" menjadi "Lithosite" di SELURUH aplikasi -- termasuk header laporan PDF Rekonsiliasi, tanda tangan email otomatis ke member, header dokumen JSA, watermark cetak, dan seluruh teks UI. Dikerjakan menyeluruh (bukan sebagian) krn nama lama berisiko sengketa/klaim pihak lain.`, en: `Full rebrand from "Geobank"/"Geobank Minerals" to "Lithosite" across the ENTIRE application -- including the Reconciliation PDF report header, automated member email signature, JSA document header, print watermark, and all UI text. Done comprehensively (not partially) since the old name carried dispute/claim risk from another party.` },
  { id: `Logo baru Lithosite (bentuk berlian/gem berfaset) dipasang di 3 titik brand utama: Mobile Top Bar, Header Sidebar utama (menggantikan ikon pickaxe generik), dan footer Profil Developer. 2 kemunculan ikon pickaxe lain (kartu ringkasan Digging, header modal Form Member) sengaja dipertahankan krn itu ikon dekoratif fitur, bukan representasi brand.`, en: `New Lithosite logo (faceted diamond/gem shape) installed at 3 main brand touchpoints: Mobile Top Bar, main Sidebar Header (replacing the generic pickaxe icon), and the Developer Profile footer. 2 other pickaxe icon occurrences (Digging summary card, Form Member modal header) intentionally kept since those are decorative feature icons, not brand representation.` },
  { id: `Set lengkap ikon PWA/favicon (favicon.ico, favicon-16, favicon-32, apple-touch-icon, icon-192, icon-512) di-generate ulang dari logo Lithosite. apple-touch-icon diberi background gelap solid (bukan transparan) krn iOS merender transparansi jadi hitam polos kalau dibiarkan.`, en: `Complete set of PWA/favicon icons (favicon.ico, favicon-16, favicon-32, apple-touch-icon, icon-192, icon-512) regenerated from the Lithosite logo. apple-touch-icon given a solid dark background (not transparent) since iOS renders transparency as plain black if left as-is.` },
  { id: `Fitur baru: Background Animasi (opsional, default nonaktif) -- kanvas 42 titik saling terhubung bergerak halus di latar belakang, dikontrol dari Developer Console > Technical (Aktif/Nonaktif + cakupan tampil: Login saja / Login+Settings / Semua Halaman). Otomatis nonaktif kalau perangkat mengaktifkan "Kurangi Animasi". Animasi benar-benar berhenti (bukan cuma disembunyikan) saat tidak relevan, demi hemat baterai/CPU di perangkat lapangan.`, en: `New feature: Background Animation (optional, off by default) -- a canvas of 42 gently-moving connected points in the background, controlled from Developer Console > Technical (On/Off + display scope: Login only / Login+Settings / All Pages). Automatically disabled if the device has "Reduce Motion" enabled. The animation fully stops (not just hidden) when not relevant, to save battery/CPU on field devices.` },
  { id: `Fitur baru: Splash Screen loading awal (opsional, default nonaktif) -- layar logo + progress bar bertahap ("Memuat data situs..." -> "Memuat blok pit..." -> "Kalibrasi model survei..." -> "Sistem Siap") saat aplikasi pertama dibuka, memakai kanvas Background Animasi yang sama sebagai latar. Durasi bisa diatur 1-7 detik lewat slider di Developer Console > Technical.`, en: `New feature: initial-load Splash Screen (optional, off by default) -- a logo + staged progress bar screen ("Initializing site data..." -> "Loading pit blocks..." -> "Calibrating survey models..." -> "System Online") on first app load, reusing the same Background Animation canvas underneath. Duration adjustable from 1-7 seconds via a slider in Developer Console > Technical.` }
 ] },
