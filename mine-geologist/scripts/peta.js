...
// Keep the internal SVG coordinate scale isotropic (1 unit X == 1 unit Y) while
// matching the real portrait/landscape viewport ratio. The old 320x320 viewBox
// was always square, so mobile portrait devices could letterbox the map vertically
// even though computeResponsiveDisplayBounds_() had already expanded the native
// display bounds to the correct viewport ratio. HIGH remains frozen at 320x320.
function getMapSvgViewportSize_() {
  ...
  try {
    ...
    tier = String(profile && profile.tier || '').toUpperCase();
  } catch (_) {}
  if (tier !== 'LOW') return { viewW: base, viewH: base };
