...
│       │                              GeoTIFF, KML, GeoPDF Coordinate Engine
│       ├── chat.js                 <- Modal Chat Tim
│       ├── issue.js                <- Modal Issue & Action
│       ├── settings.js             <- Report, Pengaturan, Menu Akun
│       │
│       └── map/                    <- subsystem Map/Lithosite
│           ├── map-security.js              <- security boundary, content validation, defensive rendering
│           ├── map-state.js                  <- state Map/Lithosite dan lifecycle state
│           ├── map-coordinate.js             <- koordinat, transformasi, dan referensi posisi peta
│           ├── map-device-profile.js         <- device profiler dan pemilihan capability perangkat
│           ├── map-tile-pyramid.js            <- pembangunan tile pyramid dan level/factor
│           ├── map-tile-queue.js              <- queue tile yang sudah dipilih untuk loading
│           ├── map-tile-store.js              <- penyimpanan dan pengambilan tile
│           ├── map-runtime-loader.js          <- loader tile pada runtime
│           ├── map-runtime-tile-creation.js   <- pembuatan tile untuk kebutuhan runtime
│           ├── map-library.js                 <- Map Library dan daftar peta tersimpan
│           ├── map-library-contract.js        <- contract data dan API Map Library
│           ├── map-library-capability.js     <- capability dan dukungan Map Library
│           ├── map-management-compat.js      <- management UI, modal, dan kompatibilitas Map Library
│           ├── map-ui.js                      <- UI peta dan komponen tampilan Map
│           ├── map-interaction.js             <- interaksi pengguna dengan peta
│           ├── map-package.js                 <- struktur dan model package peta
│           ├── map-package-transfer.js        <- import/export dan transfer package peta
│           ├── map-upload-save-lifecycle.js   <- lifecycle upload dan penyimpanan peta
│           ├── map-lifecycle-completion.js    <- penyelesaian lifecycle peta
│           ├── map-surface-lifecycle.js       <- lifecycle surface/renderer peta
│           ├── map-background-lifecycle.js    <- lifecycle background map
│           ├── map-storage-capability.js      <- capability storage peta
│           ├── map-storage-management.js      <- management storage, kapasitas, dan metadata
│           ├── map-safe-cleanup.js             <- cleanup aman resource peta
│           ├── map-recovery.js                 <- recovery dan pemulihan state/data peta
│           └── map-missing-detail-resolver.js <- resolver detail peta yang belum lengkap
...
## 3. Evolusi Engineering & Security Architecture

Pengembangan Lithosite dilakukan secara bertahap dengan target **naik kelas**, bukan sekadar menambah fitur atau melakukan patch per kasus.

### 3.1 Siklus Pengembangan
...
- perubahan harus berbasis source dan dependency yang jelas;
- owner setiap fungsi harus diketahui sebelum patch;
- perubahan dibuat minimal dan terukur;
- fitur yang sudah PASS/LOCKED tidak diubah tanpa alasan dependency yang terbukti;
- regression test dilakukan setelah perubahan;
- security review menjadi bagian dari engineering lifecycle;
- keputusan arsitektur dan security didokumentasikan agar dapat dipakai pada pengembangan berikutnya.

### 3.2 Security Evolution

Security global didokumentasikan pada [Security Evolution](02_Architecture/Security_Evolution.md).
...
## 5. Kebijakan Penomoran Versi (penting, sering disalahpahami)
...
## 6. Catatan Arsitektur & Keputusan Penting
...
## 7. Kalau Ada yang "Tidak Berfungsi" Padahal Kodenya Sudah Benar
...
*Dokumen ini adalah referensi developer/kontributor teknis. Riwayat perubahan per versi dikelola terpisah pada dokumentasi Version History.*
