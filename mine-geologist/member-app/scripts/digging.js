...
async function loadRingkasanData() {
  ...
  try {
    ...
    if (result.status === 'error') {
      ...
      // [DIPERBAIKI] SEBELUMNYA gerbang KETAT "harus persis hari ini" -- entri lama (mis.
      // kemarin/minggu lalu) hilang sama sekali dari Digging Log walau masih relevan utk
      // ditinjau, beda dgn Validasi yg dari awal sudah tampilkan riwayat wajar (slice(-100)
      // .reverse(), tanpa gerbang tanggal). Sekarang Digging ikut pola yg sama: SEMUA entri
      // terbaru (bukan cuma hari ini) tetap tampil, diurutkan terbaru dulu -- data lama
      // otomatis "tenggelam" ke bawah krn urutan, bukan hilang total dari tampilan.
      const rows = result.data || [];
      const sorted = rows.slice().sort((a, b) => new Date(b['Tanggal']) - new Date(a['Tanggal']));
      globalDiggingToday = sorted.slice(0, 30);
      const todayLabel = new Date().toDateString();
      const hasEntryToday = sorted.some(r => {
        const t = r['Tanggal'];
        if (!t) return false;
        const dt = new Date(t);
        return !isNaN(dt) && dt.toDateString() === todayLabel;
      });
      // diggingViewMode SEKARANG cuma pengaruh LABEL teks ("hari ini" vs "terbaru tercatat"),
      // BUKAN lagi penyaring data -- data selalu gabungan terbaru, apa pun labelnya.
      diggingViewMode = hasEntryToday ? 'today' : 'recent';
    }
  } catch (err) {
    if (mySeq !== ringkasanFetchSeq) return;
...
function renderRingkasan() {
      ...
      '<div class="flex gap-[10px] items-center flex-1 min-h-0">' +
        ...
        '<div class="flex-1 min-w-0 flex flex-col justify-center gap-[2px]">' +
          ...
          '<div class="flex items-baseline gap-1">' +
            '<span class="text-[24px] font-black leading-none text-white tracking-tight"' + valueSpanAttrs + '>' + value + '</span>' +
  ...
  if (!rows.length) {
    // Jika fetch gagal, jangan tampilkan empty-state. Data belum dapat dinyatakan kosong.
    // Error-state menjadi status utama agar kedua informasi tidak berebut ruang.
    if (!dataLoadErrorMsg) {
      html += '<div class="w-full shrink-0 rounded-[12px] bg-[#0b1329] border border-white/[0.08] p-6 text-center text-white/40 text-xs box-border">Belum ada data Digging tercatat sama sekali. Tekan tombol + di Digging utk mulai input.</div>';
    }
  } else {
    ...
    rows.forEach((r, i) => {
      ...
      html += '<button data-digging-search-text="' + searchText.replace(/&/g,'&amp;').replace(/\"/g,'&quot;') + '" data-digging-row-index="' + i + '" onclick="openDiggingDetail(' + i + ')" class="text-left min-h-[62px] rounded-[12px] bg-[#0b1329] border border-white/[0.08] p-3.5 flex items-center justify-between shrink-0 active:scale-[0.99] transition-transform">' +
        '<div class="flex items-center gap-3 min-w-0">' +
          ...
          '<div class="min-w-0">' +
            '<div class="text-white font-bold text-[13px] leading-none truncate">' + (pelapor ? (escapeHtml_(pelapor) + ' &bull; ') : '') + escapeHtml_(r['Pit']||'-') + ' &bull; ' + (parseFloat(r['Tonase'])||0).toFixed(0) + ' Ton</div>' +
            '<div class="text-[11px] text-white/45 mt-1 font-medium truncate">Ni ' + ni.toFixed(2) + '%' + (fe ? (', Fe ' + fe.toFixed(2) + '%') : '') + (sm ? (', SM ' + sm.toFixed(2)) : '') + '</div>' +
          '</div>' +
        '</div>' +
...
function renderDiggingModal(justOpened) {
  ...
  '<div class="fixed inset-0 bg-black/70 backdrop-blur-sm z-40 flex items-end' + animClass + '" onclick="if(event.target===this)closeDiggingForm()">' +
    '<div class="w-full max-w-[480px] mx-auto bg-[#0e1933] rounded-t-[28px] overflow-y-auto border-t border-white/10" style="max-height:calc(var(--mg1-vvh, 100svh) * 0.92);">' +
