...
(function(){
  ...
  function showMapActionSheet_(entry, activeId, onDone) {
    ...
    document.body.appendChild(root);
    // Pre-build the share package while the action sheet is open. The Share
    // action remains disabled until preparation completes, so a later tap can
    // invoke navigator.share() directly without an await before the call.
    const shareButton = root.querySelector('[data-action="share"]');
    if (shareButton) {
      shareButton.disabled = true;
      shareButton.style.opacity = '.55';
      shareButton.style.cursor = 'wait';
      const shareLabel = shareButton.querySelector('span:last-child');
      if (shareLabel) shareLabel.textContent = 'Menyiapkan Bagikan / Transfer Peta…';
    }
    try {
      if (window.MG1MapPackageTransfer && typeof window.MG1MapPackageTransfer.prepareShare === 'function') {
        window.MG1MapPackageTransfer.prepareShare([String(entry.id)]).then(function () {
          if (shareButton && document.body.contains(shareButton)) {
            shareButton.disabled = false;
            shareButton.style.opacity = '';
            shareButton.style.cursor = '';
            if (shareLabel) shareLabel.textContent = 'Bagikan / Transfer Peta';
          }
          ...
          if (shareButton && document.body.contains(shareButton)) {
            ...
            if (shareLabel) shareLabel.textContent = 'Bagikan / Transfer Peta tidak tersedia';
          }
