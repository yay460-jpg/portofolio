...
(function(){
  function escapeCss_(value) { return String(value).replace(/\\/g,'\\\\').replace(/"/g,'\\"'); }
...
(function(){
      ...
      <div id="mg1-manage-panel" style="position:absolute;left:0;right:0;bottom:0;max-height:82vh;background:#0e1933;border-top:1px solid rgba(255,255,255,0.10);border-radius:22px 22px 0 0;transform:translateY(100%);transition:transform 300ms cubic-bezier(0.16,1,0.3,1);overflow:auto;box-shadow:0 -18px 60px rgba(0,0,0,.28);">
        <div style="padding:18px 16px 14px;">
          ...
          <div id="mg1-manage-options-panel" style="display:none;margin:-4px 0 12px;padding:10px;background:#101d39;border:1px solid rgba(255,255,255,.08);border-radius:12px;">
            ...
            <button id="mg1-manage-storage-open" type="button" style="display:block;width:100%;margin-top:9px;height:34px;border-radius:9px;background:rgba(37,99,235,.10);border:1px solid rgba(96,165,250,.18);color:#93c5fd;font-size:9px;font-weight:800;">Kelola Penyimpanan Peta</button>
            </div>
          </div>
  ...
  async function showStorageManagement_() {
    if (!window.MG1MapStorageManagement || typeof window.MG1MapStorageManagement.overview !== 'function') return;
    const old = document.getElementById('mg1-storage-management-modal'); if (old) old.remove();
    const root = document.createElement('div');
    root.id = 'mg1-storage-management-modal';
    root.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:flex-end;justify-content:center;background:rgba(3,8,20,.66);backdrop-filter:blur(9px);-webkit-backdrop-filter:blur(9px);';
    root.innerHTML = `<div id="mg1-storage-management-panel" role="dialog" aria-modal="true" aria-label="Kelola Penyimpanan Peta" style="width:min(100%,520px);max-height:84vh;overflow:auto;background:#0e1933;border:1px solid rgba(255,255,255,.10);border-radius:22px 22px 0 0;box-shadow:0 -18px 60px rgba(0,0,0,.42);padding:14px 16px 18px;transform:translateY(110%);transition:transform 230ms cubic-bezier(.16,1,.3,1);">
      <div style="width:38px;height:4px;background:rgba(255,255,255,.20);border-radius:9999px;margin:0 auto 12px;"></div>
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;"><div><div style="font-size:14px;font-weight:800;color:#fff;">Kelola Penyimpanan</div><div style="font-size:9px;color:rgba(255,255,255,.38);margin-top:3px;">Ukuran logis payload tiap peta · peta aktif dilindungi</div></div><button type="button" data-storage-close style="width:30px;height:30px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.07);color:rgba(255,255,255,.68);font-size:17px;">×</button></div>
      <div id="mg1-storage-overview" style="padding:10px 11px;border:1px solid rgba(96,165,250,.12);border-radius:12px;background:rgba(255,255,255,.025);font-size:10px;color:rgba(255,255,255,.62);">Membaca penggunaan peta…</div>
      <div style="margin-top:10px;padding:9px 10px;border-radius:10px;background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.12);font-size:8px;line-height:1.45;color:rgba(255,255,255,.42);">Safe Cleanup hanya dapat menghapus peta <b style="color:rgba(255,255,255,.68);">tidak aktif</b>. Peta aktif tidak dapat dipilih.</div>
      <div id="mg1-storage-items" style="margin-top:10px;"></div>
      <div style="display:flex;gap:8px;margin-top:12px;"><button type="button" data-storage-select-largest style="flex:1;height:38px;border-radius:10px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.78);font-size:9px;font-weight:800;">Pilih Peta Terbesar</button><button type="button" data-storage-cleanup style="flex:1;height:38px;border-radius:10px;background:#e11d48;border:1px solid rgba(255,255,255,.06);color:#fff;font-size:9px;font-weight:800;">Hapus yang Dipilih</button></div>
      <div style="margin-top:9px;text-align:center;font-size:8px;color:rgba(255,255,255,.25);">Tidak ada penghapusan otomatis. Setiap cleanup memerlukan konfirmasi.</div>
    </div>`;
    document.body.appendChild(root);
    const panel = root.querySelector('#mg1-storage-management-panel');
    const close = () => { panel.style.transform='translateY(110%)'; setTimeout(()=>root.remove(),230); };
    root.querySelector('[data-storage-close]').onclick = close;
    root.addEventListener('click', ev => { if(ev.target === root) close(); });

    let overview;
    try { overview = await window.MG1MapStorageManagement.overview(); }
    catch (e) { root.querySelector('#mg1-storage-overview').textContent = 'Gagal membaca penggunaan penyimpanan: ' + String(e && e.message || e); requestAnimationFrame(()=>panel.style.transform='translateY(0)'); return; }

    const fmt = n => n >= 1024*1024 ? (n/1024/1024).toFixed(1)+' MB' : n >= 1024 ? (n/1024).toFixed(1)+' KB' : n+' B';
    const storage = overview.storage;
    root.querySelector('#mg1-storage-overview').innerHTML = `<div style="display:flex;justify-content:space-between;gap:10px;"><b style="color:rgba(255,255,255,.84);">${overview.mapCount} peta</b><span>Payload logis: ${overview.totalPayloadMB.toFixed(1)} MB</span></div><div style="margin-top:4px;color:rgba(255,255,255,.34);">Penggunaan browser: ${storage && storage.estimateAvailable ? (storage.usageBytes/1024/1024).toFixed(1)+' MB' : 'tidak tersedia'} · Terbesar: ${overview.largest ? fmt(overview.largest.bytes) : '—'}</div>`;

    const itemsEl = root.querySelector('#mg1-storage-items');
    itemsEl.innerHTML = overview.items.length ? overview.items.map(function(item, idx){
      const safeId = escapeHtml_(item.id); const safeName = escapeHtml_(item.name);
      const disabled = item.active ? 'disabled' : '';
      return `<label style="display:flex;align-items:center;gap:9px;padding:9px 8px;margin-bottom:6px;border-radius:11px;background:${item.active?'rgba(16,185,129,.06)':'rgba(255,255,255,.035)'};border:1px solid ${item.active?'rgba(52,211,153,.20)':'rgba(255,255,255,.055)'};cursor:${item.active?'not-allowed':'pointer'};"><input type="checkbox" data-storage-check="${safeId}" ${disabled} style="width:15px;height:15px;accent-color:#2563eb;flex:0 0 auto;"><span style="min-width:0;flex:1;"><span style="display:block;font-size:10px;font-weight:800;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${safeName}</span><span style="display:block;margin-top:2px;font-size:8px;color:rgba(255,255,255,.34);">${fmt(item.bytes)} · ${item.active ? 'AKTIF — dilindungi' : 'siap untuk cleanup'}</span></span><span style="font-size:8px;color:rgba(255,255,255,.25);">#${idx+1}</span></label>`;
    }).join('') : '<div style="padding:24px;text-align:center;color:rgba(255,255,255,.32);font-size:10px;">Belum ada peta.</div>';

    root.querySelector('[data-storage-select-largest]').onclick = function(){
      const candidates = overview.items.filter(i => !i.active);
      root.querySelectorAll('[data-storage-check]').forEach(cb => cb.checked = false);
      if (candidates[0]) { const cb = Array.from(root.querySelectorAll('[data-storage-check]')).find(function(el){ return el.getAttribute('data-storage-check') === String(candidates[0].id); }); if(cb) cb.checked = true; }
    };

    root.querySelector('[data-storage-cleanup]').onclick = async function(){
      const ids = Array.from(root.querySelectorAll('[data-storage-check]:checked')).map(cb => cb.getAttribute('data-storage-check')).filter(Boolean);
      if (!ids.length) { if(typeof window.MG1LithositeToast === 'function') window.MG1LithositeToast('Pilih minimal satu peta yang tidak aktif', 'error'); return; }
      let plan;
      try { plan = await window.MG1MapSafeCleanup.plan(ids); }
      catch(e) { if(typeof window.MG1LithositeToast === 'function') window.MG1LithositeToast(String(e && e.message || e), 'error'); return; }
      const confirmed = await new Promise(function(resolve){
        const c = document.createElement('div'); c.id='mg1-safe-cleanup-confirm'; c.style.cssText='position:fixed;inset:0;z-index:2147483648;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(0,0,0,.72);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);';
        c.innerHTML='<div style="width:min(100%,390px);background:#0e1933;border:1px solid rgba(255,255,255,.10);border-radius:18px;box-shadow:0 20px 60px rgba(0,0,0,.45);padding:18px;color:#fff;"><div style="font-size:14px;font-weight:800;">Konfirmasi Safe Cleanup</div><div style="margin-top:7px;font-size:10px;line-height:1.55;color:rgba(255,255,255,.48);">Hapus <b style="color:#fff;">'+plan.count+' peta</b> tidak aktif dengan total payload sekitar <b style="color:#fff;">'+plan.mb.toFixed(1)+' MB</b>? Tindakan ini menghapus data peta dari perangkat.</div><div style="display:flex;gap:9px;margin-top:14px;"><button type="button" data-clean-cancel style="flex:1;height:40px;border-radius:10px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.09);color:rgba(255,255,255,.78);font-size:11px;font-weight:800;">Batal</button><button type="button" data-clean-ok style="flex:1;height:40px;border-radius:10px;background:#e11d48;border:0;color:#fff;font-size:11px;font-weight:800;">Hapus Peta</button></div></div>';
        document.body.appendChild(c); const done=v=>{c.remove();resolve(v);}; c.querySelector('[data-clean-cancel]').onclick=()=>done(false); c.querySelector('[data-clean-ok]').onclick=()=>done(true); c.onclick=e=>{if(e.target===c)done(false);};
      });
      if(!confirmed) return;
      const btn = root.querySelector('[data-storage-cleanup]'); btn.disabled=true; btn.textContent='Menghapus…';
      try {
        const result = await window.MG1MapSafeCleanup.execute(ids);
        if(typeof window.MG1LithositeToast === 'function') window.MG1LithositeToast(result.deletedCount === result.requestedCount ? 'Cleanup berhasil — ruang penyimpanan telah dibebaskan' : 'Cleanup selesai sebagian — cek daftar peta', result.deletedCount === result.requestedCount ? 'success' : 'error');
        close(); setTimeout(()=>openManageModal(),240);
        console.log('[V24.5 S2.7] Safe Cleanup PASS', result);
      } catch(e) {
        btn.disabled=false; btn.textContent='Hapus yang Dipilih';
        console.error('[V24.5 S2.7] Safe Cleanup failed', e);
        if(typeof window.MG1LithositeToast === 'function') window.MG1LithositeToast('Safe Cleanup gagal — ' + String(e && e.message || e), 'error');
      }
    };
    requestAnimationFrame(()=>requestAnimationFrame(()=>{ panel.style.transform='translateY(0)'; }));
  }
  ...
  function openManageModal() {
    ...
    async function refreshStorageSummary_() {
      ...
      try {
        ...
        console.log('[V24.5 S2.6] Storage Management UI ready');
      } catch (e) {
        if (storageStateEl) {
    ...
    const storageOpenBtn = el.querySelector('#mg1-manage-storage-open');
    if(storageOpenBtn && !storageOpenBtn.__mg1Bound) {
      storageOpenBtn.__mg1Bound = true;
      storageOpenBtn.addEventListener('click', function(){ showStorageManagement_(); });
    }
    ...
    return new Promise((resolve,reject)=>{
      ...
      const workerCode=`
        self.onmessage=function(ev){
          const d=ev.data||{}; const req=indexedDB.open(d.dbName,4);
          req.onupgradeneeded=function(){const db=req.result; if(!db.objectStoreNames.contains(d.storeName)) db.createObjectStore(d.storeName,{keyPath:'id'}); if(!db.objectStoreNames.contains('kmlOverlays')) db.createObjectStore('kmlOverlays',{keyPath:'id'}); if(!db.objectStoreNames.contains('layers')){const ls=db.createObjectStore('layers',{keyPath:'id'}); ls.createIndex('mapId','mapId',{unique:false});} if(!db.objectStoreNames.contains('mapLayerState')) db.createObjectStore('mapLayerState',{keyPath:'mapId'}); if(!db.objectStoreNames.contains('features')){const fs=db.createObjectStore('features',{keyPath:'id'}); fs.createIndex('layerId','layerId',{unique:false});}};
