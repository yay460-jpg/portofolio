...
(function(){
      ...
      <div id="mg1-manage-panel" style="position:absolute;left:0;right:0;bottom:0;max-height:82vh;background:#0e1933;border-top:1px solid rgba(255,255,255,0.10);border-radius:22px 22px 0 0;transform:translateY(100%);transition:transform 300ms cubic-bezier(0.16,1,0.3,1);overflow:auto;box-shadow:0 -18px 60px rgba(0,0,0,.28);">
        <div style="padding:18px 16px 14px;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <div>
              <div style="font-size:14px;font-weight:800;color:#fff;letter-spacing:-.01em;">Kelola Peta Background</div>
          ...
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;">
            ...
            <button id="mg1-manage-scope" type="button" aria-label="Tampilkan peta aktif" title="Aktif / Semua" data-scope="all" style="flex:0 0 48px;width:48px;height:44px;border-radius:12px;background:rgba(245,158,11,.13);border:1px solid rgba(245,158,11,.28);color:#fbbf24;font-size:20px;line-height:1;font-weight:800;">✓</button>
            <div style="position:relative;flex:1;min-width:0;">
              <input id="mg1-manage-search" type="search" autocomplete="off" placeholder="Cari peta..." aria-label="Cari peta background" style="width:100%;height:44px;box-sizing:border-box;background:#0a142b;border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:0 46px 0 14px;color:#fff;font-size:12px;outline:none;">
          ...
          <div id="mg1-manage-options-panel" style="display:none;margin:-4px 0 12px;padding:10px;background:#101d39;border:1px solid rgba(255,255,255,.08);border-radius:12px;">
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
              ...
              <label style="display:block;font-size:9px;color:rgba(255,255,255,.42);font-weight:700;">Folder
                <select id="mg1-manage-folder" aria-label="Filter folder" style="display:block;width:100%;margin-top:5px;background:#16233f;border:1px solid rgba(255,255,255,.09);border-radius:8px;padding:8px;color:#fff;font-size:10px;font-weight:700;outline:none;">
  ...
  function renderManageList_(listEl, maps, activeId) {
    ...
    listEl.innerHTML = maps.map(m => {
      ...
      const metadata = `${safeId.slice(0,12)} • ${m.tilePyramid ? (m.tilePyramid.levels?.length||0)+' level' : 'single'}${m.folderName ? ` • 📁 ${escapeHtml_(m.folderName)}` : ''}${Array.isArray(m.collectionNames) && m.collectionNames.length ? ` • ${m.collectionNames.slice(0,2).map(function(v){ return '◈ ' + escapeHtml_(v); }).join(' · ')}` : ''}`;
      return `
  ...
  function showMapActionSheet_(entry, activeId, onDone) {
    ...
    root.innerHTML = `<div id="mg1-map-action-panel" role="dialog" aria-modal="true" aria-label="Aksi peta" style="width:min(100%,520px);background:#101c38;border:1px solid rgba(255,255,255,.10);border-radius:22px 22px 0 0;box-shadow:0 -18px 55px rgba(0,0,0,.35);padding:12px 18px 18px;transform:translateY(110%);transition:transform 220ms cubic-bezier(.16,1,.3,1);">
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:0 2px 12px;">
        ...
        <div style="min-width:0;"><div style="font-size:14px;font-weight:800;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml_(entry.name || 'Tanpa nama')}</div><div style="font-size:10px;color:rgba(255,255,255,.40);margin-top:3px;">${escapeHtml_(entry.id || '')}</div></div>
      </div>
      <div style="overflow:hidden;border:1px solid rgba(255,255,255,.08);border-radius:15px;background:#091329;">
        ...
        ${row('□','Pindah Folder','data-action="folder"')}
    ...
    root.querySelectorAll('[data-action]').forEach(btn => {
      ...
      btn.onclick = async () => {
        ...
        else if (action === 'folder') showMapFolder_(entry, onDone);
