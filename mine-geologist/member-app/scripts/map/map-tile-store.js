      ...
      clean.levels = pyramid.levels.map(level => {
        ...
        if (Array.isArray(level.tiles)) {
          cl.tiles = level.tiles.map(t => {
            ...
            if (t.dataUrl && typeof t.dataUrl === 'string' && globalThis.MG1LithositeSecurity && globalThis.MG1LithositeSecurity.isSafeImageDataUrl(t.dataUrl)) ct.dataUrl = t.dataUrl;
            return ct;
          }).filter(Boolean);
        } else {
          cl.tiles = [];
