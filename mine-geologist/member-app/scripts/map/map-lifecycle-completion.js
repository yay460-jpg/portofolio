...
(function (global) {
  ...
  async function duplicate(id) {
    ...
    await persist_(copy);
    try {
      if (global.MG1MapLayerLibrary && typeof global.MG1MapLayerLibrary.cloneLayersForMap === 'function') {
        await global.MG1MapLayerLibrary.cloneLayersForMap(source.id, newId);
      }
    } catch (layerError) {
      // Best-effort rollback of the newly created Map. V25.6 keeps the
      // existing V24.4 duplicate contract intact while preventing a
      // duplicate Map from silently losing its Layer subtree.
      try { await global.dbDeleteMap_(newId); } catch (_) {}
      throw layerError;
    }
