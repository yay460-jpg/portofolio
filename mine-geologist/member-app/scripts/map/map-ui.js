...
function switchPetaViewMode_(mode) {
  ...
  // Compatibility bridge: the canonical Peta/3D view owner is
  // map-topography.js. If its fixed view controller is mounted, legacy
  // callers must delegate instead of creating a second lifecycle/render path.
  if (typeof window.setMemberTopoView_ === 'function' &&
      document.getElementById('mg1-peta-view-switch')) {
    window.setMemberTopoView_(mode);
    return;
  }
