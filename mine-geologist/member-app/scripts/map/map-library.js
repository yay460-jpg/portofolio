...
(function (global) {
  ...
  async function updateMetadata(id, patch) {
    ...
    // not accepted here so rename/label/collection cannot accidentally
    // mutate tiles, GeoReference, imageDataUrl, or tilePyramid.
    const updated = Object.assign({}, current);
    ['name', 'labels', 'collectionNames'].forEach(function (key) {
      if (Object.prototype.hasOwnProperty.call(patch, key)) {
        if (key === 'name') {
          ...
          const seen = Object.create(null), labels = [];
          values.forEach(function (v) {
            const label = String(v == null ? '' : v).trim().slice(0, 80);
            if (!label) return;
            const k = label.toLocaleLowerCase();
            if (seen[k]) return;
            seen[k] = true; labels.push(label);
          });
          if (labels.length) updated.labels = labels.slice(0, 12);
          else delete updated.labels;
          // Legacy compatibility: once labels are explicitly written, stop carrying the old folder field.
