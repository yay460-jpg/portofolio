...
function renderChatModal(justOpened) {
  ...
  if (chatLoading) {
    ...
    list = '<div class="rounded-[12px] bg-rose-500/10 border border-rose-500/25 p-4 m-4"><div class="text-xs font-bold text-rose-300">Gagal memuat Chat</div><div class="text-[11px] text-rose-300/70 mt-1">' + escapeHtml_(chatErrorMsg) + '</div></div>';
    ...
    list = chatMessages.map(m => {
      ...
      return '<div class="mb-3 flex ' + (isMe ? 'justify-end' : 'justify-start') + '">' +
        '<div class="max-w-[78%] ' + (isMe ? 'bg-[#2563eb]' : 'bg-[#0b1329] border border-white/[0.08]') + ' rounded-[14px] px-3.5 py-2.5">' +
          (!isMe ? '<div class="text-[10px] font-bold text-blue-300 mb-0.5">' + escapeHtml_(m.sender||'Anonim') + (m.role ? (' &bull; ' + escapeHtml_(m.role)) : '') + '</div>' : '') +
          '<div class="text-[13px] text-white leading-snug break-words">' + escapeHtml_(m.message||'') + '</div>' +
          '<div class="text-[9px] ' + (isMe ? 'text-white/60' : 'text-white/30') + ' mt-1 text-right">' + escapeHtml_(m.timestamp||'') + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  }
  ...
  '<div class="fixed inset-0 bg-black/70 backdrop-blur-sm z-40 flex items-end' + animClass + '" onclick="if(event.target===this)closeChatModal()">' +
    '<div class="w-full max-w-[480px] mx-auto bg-[#0e1933] rounded-t-[24px] flex flex-col border-t border-white/10" style="height:calc(var(--mg1-vvh, 100svh) * 0.80);">' +
