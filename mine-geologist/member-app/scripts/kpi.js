...
function renderKpiModal(justOpened) {
  ...
  if (kpiLoading) {
    ...
    function row(label, val, color, sub) {
      return '<div class="flex items-center justify-between py-2 border-b border-white/[0.06]"><span class="text-[11px] text-white/50 font-medium">' + escapeHtml_(label) + '</span><span class="text-right"><span class="text-[13px] font-bold ' + (color||'text-white') + '">' + escapeHtml_(val === null || val === undefined || val === '' ? '-' : val) + '</span>' + (sub ? '<div class="text-[9px] text-white/30 mt-0.5">' + escapeHtml_(sub) + '</div>' : '') + '</span></div>';
    }
    ...
    '<div class="rounded-[12px] bg-[#0b1329] border border-white/[0.08] p-4">' +
      '<div class="text-white font-bold text-sm mb-1">' + escapeHtml_(kpiData.nama||'-') + '</div>' +
      '<div class="text-[11px] text-white/40 mb-3">' + escapeHtml_(kpiData.jabatan||'-') + ' &bull; ' + escapeHtml_(kpiData.status||'-') + '</div>' +
