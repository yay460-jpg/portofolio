...
const LITHOSITE_CHANGELOG_PREVIEW = [
  {
    version: 'V24.2', tanggal: '13 Sep 2026',
    items: [
      { id: 'LOW_42PCT_VIEWPORT_LOCK', en: 'Locked LOW-device viewport fit at 42% after field validation.' },
      { id: 'MARKER_SOLID_25M', en: 'Blue validation and yellow Map Tap markers become solid at <=25 m detail and return to default outside the threshold.' }
    ]
  },
  {
...
function renderLithositeChangelogItems_(items) {
  const lithositeLang = document.documentElement.lang === 'en' ? 'en' : 'id';
  if (!Array.isArray(items) || !items.length) {
    return '<div class="text-[10px] text-white/30">Tidak ada catatan perubahan.</div>';
  }
  return items.map(function(item) {
    const text = lithositeLang === 'en'
      ? (item.en || item.id || '')
      : (item.id || item.en || '');
...
function renderAccountMenu() {
  ...
  return '' +
  '<div class="mg1-account-overlay absolute inset-0 z-40" onclick="closeAccountMenu()">' +
