...
function renderValidasi() {
  ...
  if (!groups.length) {
    ...
    groups.forEach((g, i) => {
      const headerLine = escapeHtml_(g.idTp) + ' &bull; ' + escapeHtml_(g.depthCount) + '/' + escapeHtml_(g.maxDepth) + ' m &bull; ' + escapeHtml_(g.area||'-') + ' &bull; Bench ' + escapeHtml_(g.bench||'-');
      function assayStat(label, val, accent) {
        return '<div class="flex-1 min-w-0"><div class="text-[9px] font-bold text-white/35 tracking-wide">' + escapeHtml_(label) + '</div><div class="text-[13px] font-bold ' + (accent ? 'text-[#2563eb]' : 'text-white') + ' mt-0.5">' + escapeHtml_(val) + '</div></div>';
      }
...
function renderValidasiDetailModal(justOpened) {
  ...
  function infoCard(label, val) {
    return '<div class="rounded-[10px] bg-[#0b1329] border border-white/[0.08] px-3 py-2.5"><div class="text-[13px] font-bold text-white">' + escapeHtml_(val || '-') + '</div><div class="text-[10px] text-white/40">' + escapeHtml_(label) + '</div></div>';
  }
  ...
  g.depths.forEach((d, idx) => {
    ...
    body += '<div class="px-3.5 py-3 ' + (idx > 0 ? 'border-t border-white/[0.06]' : '') + '">' +
      '<div class="flex items-center justify-between mb-1.5">' +
        '<span class="text-[12px] font-black text-white">' + escapeHtml_(meterVal) + ' m</span>' +
      ...
      (catatan ? '<div class="text-[10px] text-white/35 mt-1 italic">' + escapeHtml_(catatan) + '</div>' : '') +
    '</div>';
  });
  body += '</div>';
  ...
  '</div>';
  return renderSimpleModal('Detail Test Pit', escapeHtml_(g.idTp), body, 'closeValidasiDetail()', undefined, justOpened);
}
