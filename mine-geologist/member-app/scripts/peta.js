...
// STEP C2 — Engine V2 automatic runtime activation.
if (typeof window.mg1AdaptiveC2Enabled !== 'boolean') window.mg1AdaptiveC2Enabled = true;
// Engine V2: C2 runtime activation is automatic; no manual upload toggle.
...
// STEP C2 - ADAPTIVE VISIBLE TILE RENDER V2
// Engine V2 C2 runtime is enabled automatically by the activation guard above.
...
// factor profile, atau ukuran tile. Identity ini menjadi fondasi cache/queue berikutnya.
...
// === V24.5 WAKIL + GUARD: single-source quality selection ===
// C1 supplies geometry demand only. This layer selects quality, then validates
// capability before Queue/Loader are allowed to consume the result.
function selectLithositeQualityWithGuard_(visibleKeys, detailLevel, engineProfile, options) {
  const policy = (typeof window !== 'undefined' && window.TILE_BUDGET_POLICY_) || { QUALITY_FLOOR:50, MAX_EXPANSION_RADIUS:20 };
  const floor = Math.max(0, Math.floor(Number(options && options.qualityFloor) || Number(policy.QUALITY_FLOOR) || 50));
  const radius = Math.min(20, Math.max(0, Math.floor(Number(options && options.maxExpansionRadius) || Number(policy.MAX_EXPANSION_RADIUS) || 20)));
  const profile = engineProfile || (typeof window !== 'undefined' ? window.mg1DeviceTileEngineProfile : null) || {};
  const maxTiles = Math.max(0, Math.floor(Number(profile.maxTiles) || 0));
  const effectiveCeiling = Math.max(floor, maxTiles);
  const capabilityCeiling = maxTiles > 0 ? maxTiles : effectiveCeiling;
  const input = Array.isArray(visibleKeys) ? visibleKeys.map(String).filter(Boolean) : [];
  const uniqueVisible = Array.from(new Set(input));
  const result = {
    ok: true,
    selectedKeys: [],
    visibleKeys: uniqueVisible.slice(),
    prefetchKeys: [],
    selectedCount: 0,
  ...
  if (!uniqueVisible.length) return result;

  // Parse only the canonical identity generated by makeLithositeTileId_.
  const parsed = uniqueVisible.map(function(key) {
    const m = /^L(-?(?:\d+(?:\.\d+)?|\.\d+))_X(-?\d+)_Y(-?\d+)$/.exec(key);
    return m ? { key, factor:Number(m[1]), x:Number(m[2]), y:Number(m[3]) } : null;
  });
  if (parsed.some(x => !x)) {
    return Object.assign(result, { ok:false, reason:'invalid tile identity', selectedKeys:[] });
  }

  // Visible coverage is mandatory even when it exceeds capability. Never trim it.
  // Effective policy ceiling expresses the requested floor/capability relationship,
  // but the Raja capability ceiling remains authoritative for actual selection.
  // Therefore floor=50 with maxTiles=40 is reported as a conflict and cannot
  // silently raise the device capability from 40 to 50.
  const target = Math.max(uniqueVisible.length, Math.min(floor, effectiveCeiling));
  const safeTarget = maxTiles > 0
    ? Math.max(uniqueVisible.length, Math.min(target, maxTiles))
    : target;
  if (floor > maxTiles && maxTiles > 0) result.conflict = 'QUALITY_FLOOR_EXCEEDS_CAPABILITY';
  if (uniqueVisible.length > capabilityCeiling && capabilityCeiling > 0) result.conflict = 'VISIBLE_EXCEEDS_CAPABILITY';

  const candidates = [];
  const seen = new Set(uniqueVisible);
  const level = detailLevel || {};
  const tilesX = Math.max(1, Math.ceil(Number(level.width || 0) / Math.max(1, Number((window.mg1DeviceTileEngineProfile || {}).tileSize || 768))));
  const tilesY = Math.max(1, Math.ceil(Number(level.height || 0) / Math.max(1, Number((window.mg1DeviceTileEngineProfile || {}).tileSize || 768))));
  const minX = Math.min.apply(null, parsed.map(x => x.x));
  const maxX = Math.max.apply(null, parsed.map(x => x.x));
  const minY = Math.min.apply(null, parsed.map(x => x.y));
  const maxY = Math.max.apply(null, parsed.map(x => x.y));
  const factor = parsed[0].factor;
  for (let d = 1; d <= radius && uniqueVisible.length + candidates.length < safeTarget; d++) {
    for (let y = Math.max(0, minY-d); y <= Math.min(tilesY-1, maxY+d); y++) {
      for (let x = Math.max(0, minX-d); x <= Math.min(tilesX-1, maxX+d); x++) {
        if (x > minX && x < maxX && y > minY && y < maxY) continue;
        if (Math.max(Math.max(minX-x, x-maxX), Math.max(minY-y, y-maxY)) !== d) continue;
        const key = typeof makeLithositeTileId_ === 'function' ? makeLithositeTileId_(factor, x, y) : null;
        if (key && !seen.has(key)) { seen.add(key); candidates.push({ key, x, y, distance:d }); }
        if (uniqueVisible.length + candidates.length >= safeTarget) break;
      }
      if (uniqueVisible.length + candidates.length >= safeTarget) break;
    }
  }
  const prefetch = candidates.slice(0, Math.max(0, safeTarget - uniqueVisible.length)).map(x => x.key);
  result.prefetchKeys = prefetch;
  result.selectedKeys = uniqueVisible.concat(prefetch);
  result.selectedCount = result.selectedKeys.length;
  result.reason = prefetch.length ? 'visible-plus-nearest-prefetch' : 'visible-only';
  return result;
}
...
function startGpsTracking_() {
  ...
  // GPS boleh diaktifkan tanpa GeoReference. Jika GeoReference tersedia,
  // posisi juga diproyeksikan ke GeoPDF; jika tidak, status tetap GPS-only.
  const activeMap = activeBackgroundMapId ? backgroundMapsList.find(m => m.id === activeBackgroundMapId) : null;
  ...
  gpsWatchId_ = navigator.geolocation.watchPosition(
    pos => {
      ...
      const insideBoundary = mapped && currentGeoReference
        ...
        : false;
      const gpsStatus = !currentGeoReference
        ? 'gps-only'
        : (mapped && insideBoundary ? 'ok' : (mapped ? 'outside-neatline' : 'transform-error'));
      gpsState_ = {
        active: true, status: gpsStatus,
        ...
        error: !currentGeoReference
          ? 'GPS aktif. Peta saat ini belum memiliki GeoReference untuk pemetaan ke GeoPDF.'
          : (mapped ? (insideBoundary ? null : 'Posisi GPS berada di luar Neatline GeoPDF.') : 'Koordinat GPS tidak dapat diproyeksikan ke GeoPDF.')
      };
      requestMapRender_();
    },
    err => {
  ...
  if (activeBackgroundMapId) {
    ...
    if (activeMap) {
      ...
      if (pyramid && Array.isArray(pyramid.levels) && pyramid.levels.length) {
        ...
        // On LOW/C2 the stored detail follows the profiled detail factor. Keep it as detail
        ...
        try {
          ...
          if (pyramid && detailLevel) {
            ...
            if (visibleResult.ok) {
              const engineProfile = window.mg1DeviceTileEngineProfile || (typeof getDeviceTileEngineProfile_ === 'function' ? getDeviceTileEngineProfile_() : null);
              const approvedSelection = typeof selectLithositeQualityWithGuard_ === 'function'
                ? selectLithositeQualityWithGuard_(visibleResult.keys, detailLevel, engineProfile, { qualityFloor:50, maxExpansionRadius:20 })
                : { ok:false, selectedKeys:[], visibleKeys:visibleResult.keys, prefetchKeys:[], reason:'quality guard unavailable' };
              try { window.mg1LastApprovedSelection = approvedSelection; } catch(_) {}
              if (approvedSelection.ok && typeof reconcileLithositeTileQueueWithVisible_ === 'function') {
                reconcileLithositeTileQueueWithVisible_(pyramid, approvedSelection.selectedKeys);
              }
              if (approvedSelection.ok && approvedSelection.selectedKeys.length) {
                const runtimeResult = typeof ensureRuntimeTiles_NonBlocking_ === 'function' ? ensureRuntimeTiles_NonBlocking_(approvedSelection.selectedKeys, pyramid) : { ok:false, queued:0, missing:[], alreadyReady:0, alreadyLoading:0 };
...
function buildMapDetailModalHtml_(mapData) {
  ...
  const depthChips = depthRows.map(d => {
    ...
    return '<span class="shrink-0 px-2.5 py-1 rounded-full bg-white/[0.06] border border-white/[0.08] text-[10px] text-white/70">' + escapeHtml_(meter) + ' m' + (ni != null && ni !== '' ? ' · Ni ' + escapeHtml_(fmt2(parseFloat(ni))) + '%' : '') + '</span>';
  }).join('');
  ...
  return '<div class="fixed inset-0 flex items-end justify-center bg-black/55" onclick="closeMapDetail()">' +
    '<div class="mg1-point-detail-sheet w-full max-w-md bg-[#0b1329] border-t border-white/10 rounded-t-[20px] px-4 pt-3 pb-4 shadow-2xl overflow-y-auto" style="max-height:calc(var(--mg1-vvh,100svh) * .82);transform:translate3d(0,0,0);" onclick="event.stopPropagation()">' +
      ...
      '<div class="flex items-start justify-between gap-3 mb-3">' +
        '<div class="min-w-0"><div class="text-white font-bold text-sm leading-tight">' + escapeHtml_(p.idTp) + '</div>' +
          '<div class="text-[9px] text-white/35 mt-1">' + escapeHtml_(dateText) + ' <span class="mx-1">•</span> Lithosite</div></div>' +
      ...
      '<div class="grid grid-cols-3 gap-1.5 mb-2.5">' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Blok</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.blok||'-') + '</div></div>' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Area</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.area||'-') + '</div></div>' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Tipe</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.tipeLaterit||'-') + '</div></div>' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Bench</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.bench||'-') + '</div></div>' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Timur (X)</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.timur||'-') + '</div></div>' +
        '<div class="rounded-lg bg-white/[0.035] border border-white/[0.06] px-2 py-2"><div class="text-[8px] text-white/35">Utara (Y)</div><div class="text-[10px] text-white font-semibold truncate mt-0.5">' + escapeHtml_(p.utara||'-') + '</div></div>' +
      '</div>' +
      photoHtml +
      ...
      '<div class="flex items-center gap-2 mt-3 py-2 border-t border-white/[0.06]"><span class="text-[9px] text-white/35 shrink-0">Catatan</span><span class="text-[10px] text-white/70 truncate">' + escapeHtml_(note) + '</span></div>' +
...
/* V24.3 STRUCTURAL CLEANUP: Map Management UX bridge moved to map/map-management-compat.js.
 * Behavior intentionally preserved; this file remains the engine/coordinate owner.
 */
