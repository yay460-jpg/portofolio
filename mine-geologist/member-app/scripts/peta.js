...
let mapRotationDeg_ = 0;     // STEP 7.6F: persistent map rotation, driven only by dual-finger gesture.
...
function scheduleMapViewportFit_() {
  ...
  // FIX GLITCH: jangan rebuild saat modal detail terbuka - itu bikin flicker rebuild
  if ((typeof mapDetailIdTp !== 'undefined' && mapDetailIdTp) || mapDetailClosing_) return;
  ...
  requestAnimationFrame(() => {
    ...
    // FIX GLITCH: double-check modal masih tertutup sebelum render
    if ((typeof mapDetailIdTp !== 'undefined' && mapDetailIdTp) || mapDetailClosing_) return;
...
function focusMapFromValidasi(idTp) {
  mapFocusIdTp = idTp;
  switchTab('peta');
  // switchTab() melakukan render() dan renderPeta() hanya menyiapkan state focus.
  // Tampilkan modal setelah tab selesai dirender, tanpa rebuild map sekali lagi.
  if (idTp) {
...
async function persistLithositeRuntimeTileStore_(pyramid) {
  ...
  try {
    // V24.1 race guard: jangan overwrite jika entry di RAM sudah lebih baru
    try {
      const curVer = Number(entry.tilePyramid && entry.tilePyramid.__persistVersion) || 0;
      const newVer = Number(pyramid.__persistVersion) || 0;
      // Jika newVer < curVer dan curVer bukan 0, skip (versi lama)
...
// ==== FIX GLITCH REBUILD MODAL - modal decoupled dari renderPeta ====
// Modal lifecycle sengaja dipisah dari render() global.
// Enter: bottom-up, TANPA fade. Close: fade-out ringan (sesuai UX yang disetujui).
...
function animateMapDetailEnter_(root, shouldAnimate) {
  ...
  // Pastikan hanya panel yang bergerak. Overlay/background tidak ikut fade.
  sheet.style.transition = 'none';
  sheet.style.transform = 'translate3d(0, 100%, 0)';
  sheet.style.willChange = 'transform';
  requestAnimationFrame(() => {
    if (!document.getElementById('mg1-tp-detail-root')) return;
    requestAnimationFrame(() => {
      sheet.style.transition = 'transform 280ms cubic-bezier(.22,.9,.25,1)';
      sheet.style.transform = 'translate3d(0, 0, 0)';
      setTimeout(() => {
        try { sheet.style.willChange = 'auto'; } catch (_) {}
      }, 320);
    });
  });
}
...
function showMapDetailModal_() {
  ...
  try {
    if (mapDetailCloseTimer_) {
      clearTimeout(mapDetailCloseTimer_);
      mapDetailCloseTimer_ = null;
    }
    mapDetailClosing_ = false;

    const mapData = (typeof buildMapData === 'function') ? buildMapData() : [];
    const p = mapData.find(m => m.idTp === mapDetailIdTp);
    if (!p) { // data hilang, tutup saja tanpa render full
      const r = document.getElementById('mg1-tp-detail-root');
      if (r) {
        r.innerHTML = '';
    ...
    const wasVisible = root.style.pointerEvents === 'auto' && !!root.querySelector('.mg1-point-detail-sheet');
    const html = renderMapDetailModal(mapData);
    root.innerHTML = html;
    root.style.pointerEvents = 'auto';
    root.style.opacity = '1';
    root.style.transition = 'none';

    // Hanya animasikan INSERT pertama. Update foto/hapus foto tidak restart animasi.
    animateMapDetailEnter_(root, !wasVisible);

    // cegah scroll body saat modal terbuka
    document.body.style.overflow = 'hidden';
  } catch(e) { console.warn('show modal fail', e); }
}

function openMapDetail(idTp) {
  if (!idTp) return;
  mapDetailIdTp = idTp;
  // JANGAN panggil render() - itu yang bikin glitch rebuild!
  // Modal dirender terpisah di layer atas.
  showMapDetailModal_();
}

function closeMapDetail() {
  const root = document.getElementById('mg1-tp-detail-root');
  if (!root || !root.querySelector('.mg1-point-detail-sheet')) {
    mapDetailIdTp = null;
    mapDetailClosing_ = false;
    document.body.style.overflow = '';
    return;
  }
  if (mapDetailClosing_) return;

  // Pertahankan mapDetailIdTp selama fade agar viewport/resize guard tetap aktif.
  ...
  // jangan panggil render() full, cukup biarkan peta tetap apa adanya
}
...
// ==== TP DETAIL PHOTO (1 foto max) - elegant slot ====
function getTpPhoto_(idTp) {
  try { return localStorage.getItem('mg1_tp_photo_' + idTp) || null; } catch(_) { return null; }
}
function setTpPhoto_(idTp, dataUrl) {
  try { localStorage.setItem('mg1_tp_photo_' + idTp, dataUrl); } catch(e) { console.warn('photo save fail', e); }
}
function removeTpPhoto_(idTp) {
  try { localStorage.removeItem('mg1_tp_photo_' + idTp); } catch(_) {}
  showMapDetailModal_();
}
function handleTpPhotoUpload_(idTp, input) {
  const file = input.files && input.files[0];
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) { alert('Maks 5MB ya'); return; }
  const reader = new FileReader();
  reader.onload = function(ev) {
    setTpPhoto_(idTp, ev.target.result);
    showMapDetailModal_();
  };
  reader.readAsDataURL(file);
}
...
function renderMapDetailModal(mapData) {
  ...
  const p = mapData.find(m => m.idTp === mapDetailIdTp);
  if (!p) return '';
  const photoData = getTpPhoto_(p.idTp);
  
  // Flat single color blue - depth pills horizontal
  const depthPills = (p.depths || []).slice().sort((a,b) => (parseFloat(getField(a,'Meter'))||0) - (parseFloat(getField(b,'Meter'))||0))
    .map(d => {
      const meter = (getField(d,'Meter')||'-');
      const ni = fmt2(parseFloat(getField(d,'Ni %')||getField(d,'Ni')));
      return '<div class="flex flex-col items-center gap-1 rounded-[12px] bg-white/[0.04] border border-white/[0.08] px-2.5 py-2 min-w-[68px]">' +
        '<span class="text-[11px] text-white/50 font-medium">' + meter + ' m</span>' +
        '<span class="px-2.5 py-0.5 rounded-full text-[11px] font-bold text-white bg-[#3b82f6]">Ni ' + ni + '%</span>' +
      '</div>';
    }).join('');

  // Catatan dari depths pertama jika ada
  let catatan = '';
  try {
    const firstDepth = (p.depths||[])[0];
    catatan = firstDepth ? (getField(firstDepth,'Catatan')||getField(firstDepth,'Keterangan')||'') : '';
  } catch(_) {}
  if (!catatan) catatan = 'Banyak boulder, Ni visual tinggi';

  const photoSlot = photoData
    ? '<div class="relative w-full h-[132px] rounded-[16px] overflow-hidden bg-[#0e1a38] border border-white/10">' +
        '<img src="' + photoData + '" class="w-full h-full object-cover" />' +
        '<button onclick="removeTpPhoto_(\'' + p.idTp.replace(/'/g,"\\'") + '\')" class="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/60 border border-white/20 flex items-center justify-center text-white text-[11px] backdrop-blur-md">✕</button>' +
        '<button onclick="document.getElementById(\'mg1-tp-photo-input-' + p.idTp.replace(/[^a-zA-Z0-9_-]/g,'') + '\').click()" class="absolute bottom-2 right-2 px-2.5 py-1 rounded-full bg-[#3b82f6] text-white text-[10px] font-bold">Ganti</button>' +
      '</div>'
    : '<button onclick="document.getElementById(\'mg1-tp-photo-input-' + p.idTp.replace(/[^a-zA-Z0-9_-]/g,'') + '\').click()" class="w-full h-[132px] rounded-[16px] border border-dashed border-[#3b82f6]/60 bg-[#0b1930]/60 flex flex-col items-center justify-center gap-2 p-3 hover:border-[#3b82f6] transition-colors">' +
        '<div class="w-12 h-12 rounded-full bg-[#3b82f6] flex items-center justify-center">' +
          '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>' +
        '</div>' +
        '<div class="text-[12px] font-bold text-[#3b82f6]">Tambah Foto</div>' +
        '<div class="text-[9px] text-white/40">JPG • PNG • Maks 5MB</div>' +
      '</button>';

  // Date & User only below ID
  const dateStr = p.tanggal ? (getField({Tanggal:p.tanggal},'Tanggal')||p.tanggal) : '29 Jul 2026';
  const userStr = 'Lithosite';

  return '<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-[2px]" onclick="closeMapDetail()">' +
    '<div class="mg1-point-detail-sheet w-full max-w-md bg-[#0b1329] border-t border-white/10 rounded-t-[24px] p-5 overflow-y-auto shadow-[0_-10px_40px_rgba(0,0,0,0.5)]" style="max-height:calc(var(--mg1-vvh, 100svh) * 0.88);" onclick="event.stopPropagation()">' +
      '<div class="w-10 h-1 rounded-full bg-white/15 mx-auto mb-4"></div>' +
      // Header ID + MG + X
      '<div class="flex items-start justify-between mb-2">' +
        '<div>' +
          '<div class="text-white font-extrabold text-[20px] tracking-tight">' + p.idTp + '</div>' +
          '<div class="flex items-center gap-1.5 mt-1 text-[11px] text-white/50"><span class="w-3 h-3 rounded-full bg-white/10 flex items-center justify-center">📍</span><span>' + dateStr + '</span><span>·</span><span>' + userStr + '</span></div>' +
        '</div>' +
        '<div class="flex items-center gap-2">' +
          '<span class="px-3 py-1 rounded-full text-[11px] font-bold bg-[#3b82f6] text-white">MG</span>' +
          '<button onclick="closeMapDetail()" class="w-8 h-8 rounded-full bg-white/[0.08] border border-white/10 flex items-center justify-center text-white/70">✕</button>' +
        '</div>' +
      '</div>' +
      (p.coordConflict ? '<div class="mb-3 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-[10px] text-amber-300 font-semibold">Konflik koordinat: Timur/Utara beda.</div>' : '') +
      // 3 cards layout - RED MARKED AREA
      '<div class="grid grid-cols-3 gap-2.5 mb-3">' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Blok</div><div class="text-[14px] text-white font-bold mt-1">' + (p.blok||'-') + '</div></div>' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Area</div><div class="text-[14px] text-white font-bold mt-1">' + (p.area||'-') + '</div></div>' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Tipe</div><div class="text-[14px] text-white font-bold mt-1">' + (p.tipeLaterit||'-') + '</div></div>' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Bench</div><div class="text-[14px] text-white font-bold mt-1">' + (p.bench||'-') + '</div></div>' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Timur (X)</div><div class="text-[13px] text-white font-bold mt-1">' + (p.timur||'-') + '</div></div>' +
        '<div class="rounded-[12px] bg-white/[0.05] border border-white/[0.08] p-3"><div class="text-[10px] text-[#7aa5d6] font-medium">Utara (Y)</div><div class="text-[13px] text-white font-bold mt-1">' + (p.utara||'-') + '</div></div>' +
      '</div>' +
      // Foto - full width dashed
      '<div class="mb-4">' + photoSlot +
        '<input type="file" id="mg1-tp-photo-input-' + p.idTp.replace(/[^a-zA-Z0-9_-]/g,'') + '" accept="image/jpeg,image/png,image/jpg" class="hidden" onchange="handleTpPhotoUpload_(\'' + p.idTp.replace(/'/g,"\\'") + '\', this)" />' +
      '</div>' +
      // Kedalaman
      '<div class="mb-3"><div class="flex items-center justify-between mb-2.5"><div class="text-[11px] font-bold tracking-wide text-[#7aa5d6]">KEDALAMAN (5/5 m)</div><div class="px-2.5 py-0.5 rounded-full bg-[#3b82f6] text-white text-[10px] font-bold">5/5 m</div></div>' +
        '<div class="flex gap-2 overflow-x-auto pb-1">' + depthPills + '</div>' +
      '</div>' +
      // Catatan
      '<div class="rounded-[12px] bg-white/[0.04] border border-white/[0.06] p-3 mb-4 flex gap-2.5 items-start">' +
        '<span class="text-[10px] font-bold text-[#3b82f6] shrink-0">Catatan</span>' +
        '<span class="text-[11px] text-white/60 leading-snug">' + catatan + '</span>' +
      '</div>' +
      '<button onclick="closeMapDetail()" class="w-full py-3.5 rounded-[14px] bg-[#3b82f6] text-white text-[14px] font-bold active:scale-[0.98] transition-transform">Tutup</button>' +
    '</div>' +
  '</div>';
}
...
function renderPeta() {
  ...
  // FIX GLITCH: modal dipisah ke layer #mg1-tp-detail-root, jangan render di sini biar tidak rebuild
      ...
      <div id="mg1-manage-panel" style="position:absolute;left:0;right:0;bottom:0;max-height:78vh;background:#0e1933;border-top:1px solid rgba(255,255,255,0.1);border-radius:20px 20px 0 0;transform:translateY(100%);transition:transform 300ms cubic-bezier(0.16,1,0.3,1);overflow:auto;">
        <div style="padding:16px;">
          ...
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;">
            <button id="mg1-manage-add" style="width:36px;height:36px;flex-shrink:0;border-radius:9999px;background:#3b82f6;border:none;color:#fff;font-size:18px;font-weight:700;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(59,130,246,0.3);">+</button>
            <button id="mg1-manage-checklist" style="width:36px;height:36px;flex-shrink:0;border-radius:9999px;background:#2a9be4;border:none;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(42,155,228,0.3);">
              <span style="font-size:16px;font-weight:900;color:#fff;line-height:1;">✓</span>
            </button>
            <div style="flex:1;position:relative;">
              <input id="mg1-manage-search" placeholder="Cari peta..." style="width:100%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:10px 36px 10px 32px;font-size:12px;color:#fff;outline:none;" />
              <span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);font-size:12px;color:rgba(255,255,255,0.3);">🔍</span>
              <button id="mg1-manage-filter" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:9999px;background:rgba(255,255,255,0.06);border:none;color:rgba(255,255,255,0.5);font-size:12px;">☰</button>
            </div>
          </div>
          <div id="mg1-manage-list"></div>
          <div style="margin-top:12px;font-size:9px;color:rgba(255,255,255,0.25);text-align:center;">Peta disimpan di HP - Offline</div>
        </div>
      </div>
    `;
    document.body.appendChild(el);
        ...
        listEl.innerHTML = maps.map(m => `
          <div style="display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.04);border-radius:12px;padding:10px;margin-bottom:8px;">
            ...
            <div style="flex:1;min-width:0;">
              ...
              <div style="font-size:10px;color:rgba(255,255,255,0.4);display:flex;align-items:center;gap:6px;flex-wrap:wrap;"><span>${(m.id||'').slice(0,12)} • ${m.tilePyramid ? (m.tilePyramid.levels?.length||0)+' level' : 'single'}</span>${(typeof activeBackgroundMapId !== 'undefined' && activeBackgroundMapId === m.id) ? '<span style="display:inline-flex;align-items:center;gap:3px;color:#10b981;font-weight:700;"><span style="width:6px;height:6px;border-radius:9999px;background:#10b981;display:inline-block;"></span>AKTIF</span>' : ''}</div>
            </div>
            ...
            <button style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.08);border-radius:9999px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;gap:3px;"><span style="width:6px;height:6px;border-radius:9999px;background:#10b981;"></span><span style="width:6px;height:6px;border-radius:9999px;background:#3b82f6;"></span><span style="width:6px;height:6px;border-radius:9999px;background:#f59e0b;"></span></button>
