...
function renderIssueModal(justOpened) {
  ...
  if (issueFormOpen) {
    ...
    body = '<div class="rounded-[12px] bg-rose-500/10 border border-rose-500/25 p-4"><div class="text-xs font-bold text-rose-300">Gagal memuat Issue</div><div class="text-[11px] text-rose-300/70 mt-1">' + escapeHtml_(issueErrorMsg) + '</div></div>';
  } else {
      ...
      issueListData.forEach(it => {
        body += '<div class="rounded-[12px] bg-[#0b1329] border border-white/[0.08] p-3.5 mb-2.5">' +
          '<div class="flex items-start justify-between gap-2 mb-1">' +
            '<span class="text-[13px] font-bold text-white leading-tight">' + escapeHtml_(it.masalah||'-') + '</span>' +
            '<span class="shrink-0 text-[9px] font-black tracking-widest px-2.5 py-1 rounded-full ' + statusBadgeColor(it.status) + ' text-white">' + escapeHtml_(it.status||'Open') + '</span>' +
          '</div>' +
          '<div class="text-[11px] text-white/45">' + escapeHtml_(it.lokasi||'-') + ' &bull; PIC ' + escapeHtml_(it.pic||'-') + (it.target ? (' &bull; target ' + escapeHtml_(it.target)) : '') + '</div>' +
        '</div>';
      });
    }
  }
