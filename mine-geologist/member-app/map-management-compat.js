...
(function(){
      ...
      <div id="mg1-manage-panel" style="position:absolute;left:0;right:0;bottom:0;max-height:78vh;background:#0e1933;border-top:1px solid rgba(255,255,255,0.1);border-radius:20px 20px 0 0;transform:translateY(100%);transition:transform 300ms cubic-bezier(0.16,1,0.3,1);overflow:auto;">
        <div style="padding:16px;">
          ...
          <div style="position:relative;margin-bottom:8px;">
            <input id="mg1-manage-search" type="search" autocomplete="off" placeholder="Cari peta..." aria-label="Cari peta background" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 12px;color:#fff;font-size:11px;outline:none;" />
          </div>
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
            <div id="mg1-manage-filter" role="group" aria-label="Filter peta" style="display:flex;gap:6px;flex:1;min-width:210px;">
              <button type="button" data-filter="all" style="flex:1;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:7px 8px;color:#fff;font-size:10px;font-weight:700;">Semua</button>
              <button type="button" data-filter="active" style="flex:1;background:transparent;border:1px solid transparent;border-radius:8px;padding:7px 8px;color:rgba(255,255,255,0.5);font-size:10px;font-weight:700;">Aktif</button>
              <button type="button" data-filter="inactive" style="flex:1;background:transparent;border:1px solid transparent;border-radius:8px;padding:7px 8px;color:rgba(255,255,255,0.5);font-size:10px;font-weight:700;">Tidak aktif</button>
            </div>
            <select id="mg1-manage-sort" aria-label="Urutkan peta" style="flex:0 0 112px;background:#16233f;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:7px 8px;color:#fff;font-size:10px;font-weight:700;outline:none;">
              <option value="name-asc">Nama A–Z</option>
              <option value="name-desc">Nama Z–A</option>
              <option value="newest">Terbaru</option>
              <option value="oldest">Terlama</option>
            </select>
            <select id="mg1-manage-folder" aria-label="Filter folder" style="flex:0 0 118px;background:#16233f;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:7px 8px;color:#fff;font-size:10px;font-weight:700;outline:none;display:none;">
              <option value="__all__">Semua folder</option>
            </select>
            <select id="mg1-manage-collection" aria-label="Filter koleksi" style="flex:0 0 128px;background:#16233f;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:7px 8px;color:#fff;font-size:10px;font-weight:700;outline:none;display:none;">
  ...
  function renderManageList_(listEl, maps, activeId) {
    ...
    if(!maps || maps.length === 0) {
      listEl.innerHTML = '<div style="text-align:center;padding:20px 0;color:rgba(255,255,255,0.3);font-size:11px;">Tidak ada peta yang cocok.</div>';
      return;
    }
    listEl.innerHTML = maps.map(m => {
      const isActive = !!activeId && String(m.id) === String(activeId);
      const cardStyle = isActive
        ? 'display:flex;align-items:center;gap:10px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.42);border-radius:12px;padding:10px;margin-bottom:8px;'
        : 'display:flex;align-items:center;gap:10px;background:rgba(255,255,255,0.04);border:1px solid transparent;border-radius:12px;padding:10px;margin-bottom:8px;';
      const action = isActive
        ? '<span aria-label="Peta aktif" style="display:inline-flex;align-items:center;gap:5px;flex-shrink:0;font-size:10px;font-weight:700;color:#6ee7b7;padding:6px 8px;"><span aria-hidden="true" style="width:7px;height:7px;border-radius:50%;background:#34d399;box-shadow:0 0 0 2px rgba(52,211,153,0.10);"></span>AKTIF</span>'
        : `<button type="button" onclick="window.MG1MapLibrary.activate('${m.id}')" style="background:#2563eb;color:#fff;border:none;border-radius:8px;padding:6px 10px;font-size:10px;font-weight:700;">AKTIFKAN</button>`;
      return `
        <div style="${cardStyle}">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:10px;color:rgba(255,255,255,0.4);">${(m.id||'').slice(0,12)} • ${m.tilePyramid ? (m.tilePyramid.levels?.length||0)+' level' : 'single'}${m.folderName ? ` • 📁 ${escapeHtml_(m.folderName)}` : ''}${Array.isArray(m.collectionNames) && m.collectionNames.length ? ` • ${m.collectionNames.slice(0,2).map(function(v){ return '◈ ' + escapeHtml_(v); }).join(' · ')}` : ''}</div>
          </div>
          ${action}
          <button type="button" aria-label="Info peta ${String(m.name||'').replace(/"/g, '&quot;')}" data-map-info="${String(m.id).replace(/"/g, '&quot;')}" style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.65);border:1px solid rgba(255,255,255,0.08);border-radius:8px;width:28px;height:28px;font-weight:800;">i</button>
          <button type="button" aria-label="Ganti nama peta ${String(m.name||'').replace(/"/g, '&quot;')}" data-map-rename="${String(m.id).replace(/"/g, '&quot;')}" style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.65);border:1px solid rgba(255,255,255,0.08);border-radius:8px;width:28px;height:28px;font-weight:800;">✎</button>
          <button type="button" aria-label="Atur folder peta ${String(m.name||'').replace(/"/g, '&quot;')}" data-map-folder="${String(m.id).replace(/"/g, '&quot;')}" style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.65);border:1px solid rgba(255,255,255,0.08);border-radius:8px;width:28px;height:28px;font-weight:800;">📁</button>
          <button type="button" aria-label="Atur koleksi peta ${String(m.name||'').replace(/"/g, '&quot;')}" data-map-collection="${String(m.id).replace(/"/g, '&quot;')}" style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.65);border:1px solid rgba(255,255,255,0.08);border-radius:8px;width:28px;height:28px;font-weight:800;">◈</button>
          <button type="button" aria-label="Hapus peta ${String(m.name||'').replace(/"/g, '&quot;')}" onclick="window.MG1MapLibrary.remove('${m.id}')" style="background:rgba(244,63,94,0.15);color:#f43f5e;border:none;border-radius:8px;width:28px;height:28px;">🗑</button>
        </div>
      `;
    }).join('');
  }
      ...
      <div role="dialog" aria-modal="true" aria-label="Info peta" style="width:min(420px,100%);max-height:80vh;overflow:auto;background:#0e1933;border:1px solid rgba(255,255,255,0.1);border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,0.45);padding:16px;color:#fff;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
          ${[
            ...
            ['Preview', preview],
            ['GeoReference', geo],
            ['Disimpan', formatMapDate_(entry.uploadedAt)]
  ...
  // V24.3 Slice 12: virtual folder metadata. No folder store yet; folderName is
  // optional map metadata so organization can evolve without touching tile/runtime data.
  function collectFolderNames_(maps) {
    const names = []; const seen = new Set();
    (Array.isArray(maps) ? maps : []).forEach(function(m) {
      const name = String(m && m.folderName || '').trim(); if (!name) return;
  ...
  function showMapFolder_(entry, onDone) {
    if (!entry || !entry.id) return;
    const old = document.getElementById('mg1-map-folder-modal'); if (old) old.remove();
    const root = document.createElement('div'); root.id = 'mg1-map-folder-modal';
    root.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;background:rgba(3,8,20,.72);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);';
    root.innerHTML = `
      <div role="dialog" aria-modal="true" aria-label="Atur folder peta" style="width:min(360px,100%);background:#0e1933;border:1px solid rgba(255,255,255,.1);border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.45);padding:16px;color:#fff;">
        <div style="font-size:13px;font-weight:800;margin-bottom:4px;">Atur Folder Peta</div>
        <div style="font-size:10px;color:rgba(255,255,255,.42);margin-bottom:10px;">Organisasi metadata saja — isi peta dan runtime tidak berubah.</div>
        <input id="mg1-map-folder-input" type="text" maxlength="80" autocomplete="off" value="${escapeHtml_(entry.folderName || '')}" placeholder="Contoh: Pit A / Geology" aria-label="Nama folder" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:9px;padding:10px 11px;color:#fff;font-size:11px;outline:none;">
        <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">
          ${['Production','Exploration','Geology','Topography','Survey'].map(function(v){ return `<button type="button" data-folder-preset="${v}" style="padding:6px 8px;border-radius:8px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.7);font-size:9px;font-weight:700;">${v}</button>`; }).join('')}
          <button type="button" data-folder-clear style="padding:6px 8px;border-radius:8px;background:rgba(244,63,94,.08);border:1px solid rgba(244,63,94,.16);color:#fda4af;font-size:9px;font-weight:700;">Tanpa folder</button>
        </div>
        <div style="display:flex;gap:8px;margin-top:12px;">
          <button type="button" data-folder-cancel style="flex:1;padding:9px;border-radius:9px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.75);font-size:10px;font-weight:800;">Batal</button>
          <button type="button" data-folder-save style="flex:1;padding:9px;border-radius:9px;background:#2563eb;border:0;color:#fff;font-size:10px;font-weight:800;">Simpan</button>
        </div>
      </div>`;
    document.body.appendChild(root);
    const input = root.querySelector('#mg1-map-folder-input'); const close = () => root.remove();
    root.querySelector('[data-folder-cancel]').onclick = close; root.onclick = ev => { if (ev.target === root) close(); };
    root.querySelectorAll('[data-folder-preset]').forEach(function(btn){ btn.onclick = () => { input.value = btn.dataset.folderPreset || ''; input.focus(); }; });
    root.querySelector('[data-folder-clear]').onclick = () => { input.value = ''; input.focus(); };
    const save = async () => {
      const folderName = String(input.value || '').trim().slice(0,80); const updated = Object.assign({}, entry);
      if (folderName) updated.folderName = folderName; else delete updated.folderName;
      try {
        if (window.MG1MapLibrary && typeof window.MG1MapLibrary.updateMetadata === 'function') {
          await window.MG1MapLibrary.updateMetadata(entry.id, {folderName: folderName});
        } else {
          if (typeof dbPutMap_ === 'function') await dbPutMap_(updated);
      ...
      } catch (e) { console.warn('[V24.3 MAP LIBRARY] folder update failed', e); }
    };
    root.querySelector('[data-folder-save]').onclick = save;
    input.addEventListener('keydown', ev => { if (ev.key === 'Enter') save(); if (ev.key === 'Escape') close(); });
    requestAnimationFrame(() => { input.focus(); input.select(); });
  }
  ...
  function openManageModal() {
    ...
    function applyManageFolder_(maps, folder) {
      if (!folder || folder === '__all__') return maps;
      if (folder === '__none__') return maps.filter(m => !String(m && m.folderName || '').trim());
      const key = String(folder).trim().toLocaleLowerCase();
      return maps.filter(m => String(m && m.folderName || '').trim().toLocaleLowerCase() === key);
    }
    ...
    function syncManageFolderOptions_(maps) {
      if (!folderEl) return;
      const names = collectFolderNames_(maps); const current = manageFolder_;
      folderEl.innerHTML = '<option value="__all__">Semua folder</option><option value="__none__">Tanpa folder</option>' + names.map(function(name){ return `<option value="${escapeHtml_(name)}">📁 ${escapeHtml_(name)}</option>`; }).join('');
      const hasCurrent = Array.from(folderEl.options).some(function(o){ return o.value === current; });
      manageFolder_ = hasCurrent ? current : '__all__'; folderEl.value = manageFolder_;
      folderEl.style.display = (names.length || maps.some(function(m){ return !String(m && m.folderName || '').trim(); })) ? '' : 'none';
    }
    ...
    async function refreshManageList_(query) {
      try {
        ...
        const total = (window.MG1MapLibraryCapability && typeof window.MG1MapLibraryCapability.list === 'function')
          ...
          : maps;
        const activeId = getManageActiveId_();
        syncManageFolderOptions_(maps);
        syncManageCollectionOptions_(maps);
        const filtered = applyManageCollection_(applyManageFolder_(applyManageFilter_(maps, activeId, manageFilter_), manageFolder_), manageCollection_);
        ...
        countEl.textContent = q || manageFilter_ !== 'all' || manageSort_ !== 'name-asc' || manageCollection_ !== '__all__'
    ...
    if(folderEl && !folderEl.__mg1Bound) {
      folderEl.__mg1Bound = true;
      folderEl.addEventListener('change', function() {
        manageFolder_ = this.value || '__all__';
        refreshManageList_(searchEl ? searchEl.value : '');
      });
    }
    ...
    if(listEl && !listEl.__mg1InfoBound) {
      ...
      listEl.addEventListener('click', async function(ev) {
        const infoBtn = ev.target.closest('[data-map-info]');
        const renameBtn = ev.target.closest('[data-map-rename]');
        const folderBtn = ev.target.closest('[data-map-folder]');
        const collectionBtn = ev.target.closest('[data-map-collection]');
        if(!infoBtn && !renameBtn && !folderBtn && !collectionBtn) return;
        const btn = infoBtn || renameBtn || folderBtn || collectionBtn;
        const attr = infoBtn ? 'data-map-info' : (renameBtn ? 'data-map-rename' : (folderBtn ? 'data-map-folder' : 'data-map-collection'));
        ...
        try {
          ...
          if (infoBtn) showMapInfo_(entry);
          else if (renameBtn) showMapRename_(entry, async function(){ await refreshManageList_(searchEl ? searchEl.value : ''); });
          else if (folderBtn) showMapFolder_(entry, async function(){ await refreshManageList_(searchEl ? searchEl.value : ''); });
          else showMapCollections_(entry, async function(){ await refreshManageList_(searchEl ? searchEl.value : ''); });
        } catch (e) { console.warn('[V24.3 MAP LIBRARY] map metadata action fail', e); }
      });
    }
