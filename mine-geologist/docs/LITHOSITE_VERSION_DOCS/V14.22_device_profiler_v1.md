# MG1 / LITHOSITE — V14.22 — Device Profiler V1
...
Device capability tidak boleh ditebak dari satu setting umum.
...
```text
Device → RAM/CPU/DPR/resolution/WebGL/Canvas benchmark → LOW / BALANCED / HIGH
```

## 4. Struktur / Arsitektur Sebelum

```text
Renderer belum mempunyai device-tier profile yang terukur.
```

## 5. Struktur / Arsitektur Sesudah

```text
getDeviceTileProfile_() menjadi dasar tier device; renderer tetap unchanged.
```
