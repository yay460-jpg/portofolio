// ============================================================
// BARGING.JS -- Fitur Shipment, Loading Log, Shift Report, Sublot, Radar Chart
// ============================================================
...
function canManageBarge() {
  return typeof isDeveloperUnlocked === 'function' ? isDeveloperUnlocked() : false;
}
...
function canAssignDome() {
  const role = String(typeof getCurrentExportRole === 'function' ? getCurrentExportRole() : '').trim().toUpperCase();
  return role === 'DEVELOPER' || role === 'SUPERVISOR';
}

// ============================================================
// STATE VARIABLES (HANYA YANG SPESIFIK UNTUK BARGING)
// ============================================================
// Semua variabel global yang shared dengan main.js diakses via window.
// Hanya variabel lokal yang dideklarasikan di sini.
let sublotRadarChartInstance = null;

// ============================================================
// FETCH BARGE SHIPMENT DATA
// ============================================================
async function fetchBargeShipmentData() {
  const listEl = document.getElementById('barge-shipment-list');
  listEl.innerHTML = `<p class="text-[11px] text-slate-500 font-medium">${window.currentLang === 'en' ? 'Loading shipment data...' : 'Memuat data shipment...'}</p>`;
  try {
    const response = await fetchWithTimeout(
      window.GOOGLE_SCRIPT_READ_URL + '?sheet=bargeshipment&t=' + new Date().getTime()
    );
    const result = await response.json();
    if (result.status !== 'success') {
      throw new Error(result.message || (window.currentLang === 'en' ? 'Failed to load shipment data.' : 'Gagal memuat data shipment'));
    }
    window.globalBargeShipmentData = result.data || [];
    renderBargeShipmentList();
    if (typeof computeReconciliationMatrix === 'function') {
      computeReconciliationMatrix(); // F3/F4 butuh Total Plant
    }
    if (typeof lucide !== 'undefined' && lucide.createIcons) lucide.createIcons();
    if (typeof markDataFresh_ === 'function') markDataFresh_('Barging');
  } catch (err) {
    console.error('Gagal memuat data shipment:', err);
    if (typeof markDataStale_ === 'function') markDataStale_('Barging');
    const isTimeout = err.name === 'AbortError';
    listEl.innerHTML = `<div class="text-center py-6"><p class="text-[11px] text-rose-400 font-medium mb-2">${
      isTimeout
        ? (window.currentLang === 'en' ? 'Server not responding (timeout).' : 'Server tidak merespons (timeout).')
        : (window.currentLang === 'en' ? 'Failed to load shipment data.' : 'Gagal memuat data shipment.')
    }</p><button onclick="fetchBargeShipmentData()" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold border border-slate-700 transition-all cursor-pointer">${
...
// ============================================================
// RENDER BARGE SHIPMENT LIST
// ============================================================
function renderBargeShipmentList() {
  const listEl = document.getElementById('barge-shipment-list');
  if (!listEl) return;
  if (window.globalBargeShipmentData.length === 0) {
    listEl.innerHTML = `<p class="text-[11px] text-slate-500 font-medium">${window.currentLang === 'en' ? 'No shipments yet.' : 'Belum ada shipment.'}</p>`;
    return;
  }

  listEl.innerHTML = window.globalBargeShipmentData.map(function(s) {
    ...
    return `<div onclick="openBargeDetailModal('${noShipmentSafe}')" class="glass-card p-3.5 rounded-xl cursor-pointer hover:border-blue-500/40 transition-all border border-slate-700/60">
      ...
      <div class="flex items-center justify-between text-[11px] text-slate-400 font-medium flex-wrap gap-1">
        ...
        ${s.tonase_aktual ? `<span class="text-emerald-400 font-semibold">${window.currentLang === 'en' ? 'Actual' : 'Aktual'}: ${Number(s.tonase_aktual).toLocaleString()} ton</span>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
// ============================================================
// OPEN BARGE DETAIL MODAL
// ============================================================
async function openBargeDetailModal(noShipment) {
  const shipment = window.globalBargeShipmentData.find(function(s) { return s.no_shipment === noShipment; });
  if (!shipment) return;
  window.currentOpenBargeShipment = noShipment;
  ...
  document.getElementById('barge-detail-summary').innerHTML = '<p class="text-[11px] text-slate-500 col-span-full">' + (window.currentLang === 'en' ? 'Loading details...' : 'Memuat detail...') + '</p>';
  ...
  ['btn-open-form-loading-log', 'btn-open-form-shift-report', 'btn-open-form-sublot'].forEach(function(id) {
  ...
  try {
    const [llRes, srRes, slRes] = await Promise.all([
      fetchWithTimeout(window.GOOGLE_SCRIPT_READ_URL + '?sheet=bargeloadinglog&t=' + new Date().getTime()).then(function(r) { return r.json(); }),
      fetchWithTimeout(window.GOOGLE_SCRIPT_READ_URL + '?sheet=bargeshiftreport&t=' + new Date().getTime()).then(function(r) { return r.json(); }),
      fetchWithTimeout(window.GOOGLE_SCRIPT_READ_URL + '?sheet=bargesublot&t=' + new Date().getTime()).then(function(r) { return r.json(); })
    ]);

    // Race condition guard
    if (noShipment !== window.currentOpenBargeShipment) return;

    window.globalBargeLoadingLogData = llRes.status === 'success' ? (llRes.data || []) : [];
    window.globalBargeShiftReportData = srRes.status === 'success' ? (srRes.data || []) : [];
    window.globalBargeSublotData = slRes.status === 'success' ? (slRes.data || []) : [];
    ...
    document.getElementById('barge-detail-summary').innerHTML = '<p class="text-[11px] text-rose-400 col-span-full">' + (window.currentLang === 'en' ? 'Failed to load details.' : 'Gagal memuat detail.') + '</p>';
  }
}
...
function closeBargeDetailModal() {
  ...
  // Reset state untuk mencegah race condition
  window.currentOpenBargeShipment = null;
}

// ... sisanya tetap sama, pastikan setiap akses ke globalBarge* menggunakan window.

// EXPOSE FUNCTIONS KE GLOBAL (tidak berubah)
window.canManageBarge = canManageBarge;
window.canViewDomeHistory = canViewDomeHistory;
window.canAssignDome = canAssignDome;
window.fetchBargeShipmentData = fetchBargeShipmentData;
window.renderBargeShipmentList = renderBargeShipmentList;
window.openBargeDetailModal = openBargeDetailModal;
window.closeBargeDetailModal = closeBargeDetailModal;
window.renderBargeDetailSummary = renderBargeDetailSummary;
window.renderBargeLoadingLogTable = renderBargeLoadingLogTable;
window.renderBargeShiftReportList = renderBargeShiftReportList;
window.renderBargeSublotTable = renderBargeSublotTable;
window.openSublotRadarModal = openSublotRadarModal;
window.closeSublotRadarModal = closeSublotRadarModal;
window.openFormBargeShipmentPopup = openFormBargeShipmentPopup;
window.closeFormBargeShipmentPopup = closeFormBargeShipmentPopup;
window.submitBargeShipment = submitBargeShipment;
window.openFormBargeLoadingLogPopup = openFormBargeLoadingLogPopup;
window.closeFormBargeLoadingLogPopup = closeFormBargeLoadingLogPopup;
window.onBargeLoadingLogAreaChange = onBargeLoadingLogAreaChange;
window.updateBargeLoadingLogTonasePreview = updateBargeLoadingLogTonasePreview;
window.submitBargeLoadingLog = submitBargeLoadingLog;
window.openFormBargeShiftReportPopup = openFormBargeShiftReportPopup;
window.closeFormBargeShiftReportPopup = closeFormBargeShiftReportPopup;
window.submitBargeShiftReport = submitBargeShiftReport;
window.openFormBargeSublotPopup = openFormBargeSublotPopup;
window.closeFormBargeSublotPopup = closeFormBargeSublotPopup;
window.submitBargeSublot = submitBargeSublot;
window.submitBargeAktual = submitBargeAktual;
window.loadDomeSelectOptions = loadDomeSelectOptions;
window.toggleNewDomeForm = toggleNewDomeForm;
window.confirmNewDome = confirmNewDome;
window.updateSplitTotalInfo = updateSplitTotalInfo;

// ======= RESTORASI DARI BASELINE (fungsi hilang di MG1) =======

// [RESTORED from baseline/barging.js] closeDomeHistoryModal
 function closeDomeHistoryModal() {
 const modal = document.getElementById('dome-history-modal');
 hideModalAnimated(modal);
 }

// [RESTORED from baseline/barging.js] closeFormBargeLoadingLogPopup
function closeFormBargeLoadingLogPopup() {
 hideModalAnimated(document.getElementById('form-barge-loadinglog-popup-modal'));
 }

// [RESTORED from baseline/barging.js] closeFormBargeShiftReportPopup
function closeFormBargeShiftReportPopup() {
 hideModalAnimated(document.getElementById('form-barge-shiftreport-popup-modal'));
 }

// [RESTORED from baseline/barging.js] closeFormBargeShipmentPopup
function closeFormBargeShipmentPopup() {
 hideModalAnimated(document.getElementById('form-barge-shipment-popup-modal'));
 }

// [RESTORED from baseline/barging.js] closeFormBargeSublotPopup
function closeFormBargeSublotPopup() {
 hideModalAnimated(document.getElementById('form-barge-sublot-popup-modal'));
 }

// [RESTORED from baseline/barging.js] closeSublotRadarModal
function closeSublotRadarModal() {
 hideModalAnimated(document.getElementById('sublot-radar-modal'));
 }

// [RESTORED from baseline/barging.js] confirmNewDome
...
// [RESTORED from baseline/barging.js] loadDomeSelectOptions
...
// [RESTORED from baseline/barging.js] onBargeLoadingLogAreaChange
...
// [RESTORED from baseline/barging.js] openDomeHistoryModal
...
// [RESTORED from baseline/barging.js] openFormBargeLoadingLogPopup
 // ---- Form: Loading Log ----
...
// [RESTORED from baseline/barging.js] openFormBargeShiftReportPopup
 // ---- Form: Laporan Shift ----
...
// [RESTORED from baseline/barging.js] openFormBargeShipmentPopup
 // ---- Form: Shipment Baru ----
...
// [RESTORED from baseline/barging.js] openFormBargeSublotPopup
 // ---- Form: Sublot Aktual ----
...
// [RESTORED from baseline/barging.js] openSublotRadarModal
...
// [RESTORED from baseline/barging.js] refreshNewDomeAreaOptions
...
// [RESTORED from baseline/barging.js] renderBargeDetailSummary
...
// [RESTORED from baseline/barging.js] renderBargeLoadingLogTable
...
// [RESTORED from baseline/barging.js] renderBargeShiftReportList
...
// [RESTORED from baseline/barging.js] renderBargeSublotTable
...
// [RESTORED from baseline/barging.js] renderDomeHistoryTable
...
// [RESTORED from baseline/barging.js] submitBargeAktual
...
// [RESTORED from baseline/barging.js] submitBargeLoadingLog
...
// [RESTORED from baseline/barging.js] submitBargeShiftReport
...
// [RESTORED from baseline/barging.js] submitBargeShipment
...
// [RESTORED from baseline/barging.js] submitBargeSublot
...
// [RESTORED from baseline/barging.js] toggleNewDomeForm
...
// [RESTORED from baseline/barging.js] updateBargeLoadingLogTonasePreview
