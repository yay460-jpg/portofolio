...
## 7. Evolusi Engineering & Security Architecture
...
- security review menjadi bagian dari engineering lifecycle, bukan tahap terpisah di akhir;
- keputusan arsitektur dan security harus didokumentasikan agar dapat dipakai pada pengembangan berikutnya.

### 7.2 Security Evolution

Security global didokumentasikan pada:

`Lithosite Android/02_Architecture/Security_Evolution.md`

Halaman tersebut tidak terikat pada nama versi aplikasi dan menjadi referensi global untuk trust boundary, content validation, dan defensive rendering.
...
Data dari user, file import, API, storage, package, atau sumber eksternal harus diperlakukan sebagai **untrusted** sampai melewati kontrol yang sesuai.

Untuk data yang masuk ke DOM:

```text
Text       → textContent / value
URL        → validate scheme/content → DOM property
Tile key   → validate canonical grammar
HTML/SVG   → hanya dari komponen yang sudah trusted/validated
```

### 7.5 Target Kematangan Lithosite
...
Tujuannya bukan memperbanyak kode, tetapi membuat sistem semakin:

**jelas → terukur → aman → stabil → mudah dipelihara.**
